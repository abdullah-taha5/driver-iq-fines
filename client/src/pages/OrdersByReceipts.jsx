...
function OrdersByReceipts({ userData }) {
  ...
  useEffect(() => {
    const getOrder = async () => {
      ...
      setOrders(data.ordersByReceipts);
      setLoading(true);
    };
  ...
  const payOrder = async (id) => {
    const { data } = await axios.get(
      `${BASE_URL}/api/order-pay-receipt/pay/${id}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  if (token.get("token")) {
    const paymentStatus = async () => {
      const { data } = await axios.put(
        `${BASE_URL}/api/order-pay-receipt/status?token=${token.get("token")}`
      );
    };
    paymentStatus();
  }
...
export default OrdersByReceipts;
