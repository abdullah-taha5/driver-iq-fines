...
function AdminSignIn() {
  ...
  const adminSignIn = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`${BASE_URL}/api/auth/admin/login`, admin);
        ...
        <div className={`signin-form mt-3 ${styles.signInForm}`}>
          <div className="signin-form-inner" style={{ marginTop: "80px" }}>
            <div className="view show" id="signin-view">
              <h1 className="h2 text-center mb-4">{t("adminSignIn")}</h1>
              <form className="needs-validation" onSubmit={adminSignIn}>
