...
function Orders({ userData }) {
  ...
  useEffect(() => {
    const getOrders = async () => {
      const { data } = await axios.get(
        `${BASE_URL}/api/orders?pageNumber=${currentPage}`,
        {
          headers: {
    ...
    const getOrdersCount = async () => {
      const { data } = await axios.get(`${BASE_URL}/api/orders/count`);
      setOrdersCount(data);
    };
  ...
  const deleteOrder = async (id) => {
    const { data } = await axios.delete(`${BASE_URL}/api/orders/${id}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
  ...
  const pendingOrder = async (id, user, orderId) => {
    const { data } = await axios.put(
      `${BASE_URL}/api/orders/order-status/${id}`,
      {
        orderStatus: "Pending",
        user,
        notification: `طلبك رقم #${orderId} معلق`,
      },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const confirmOrder = async (id, user, orderId) => {
    const { data } = await axios.put(
      `${BASE_URL}/api/orders/order-status/${id}`,
      {
        orderStatus: "Confirm",
        user,
        notification: `طلبك رقم #${orderId} تم موافقته`,
      },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const completeOrder = async (id, user, orderId) => {
    const { data } = await axios.put(
      `${BASE_URL}/api/orders/order-status/${id}`,
      {
        orderStatus: "Completed",
        user,
        notification: `طلبك رقم #${orderId} مكتمل`,
      },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
    ...
    <Fragment>
      ...
      >
        <div className="row">
          ...
          {orders.length > 0 ? (
            <div className="mt-5 col-sm-9" style={{ overflowX: "auto" }}>
              <div className="row">
                  ...
                  {orders
                    ...
                    .map((order, i) => (
                      <tr key={i}>
                        ...
                        <td>{t(order.orderStatus).toLowerCase()}</td>
                        <td
                          className={`${
